...
export default function TokenCheckout() {
  ...
  // Get plan from URL params or default to value
  ...
  const handlePayment = useCallback(async () => {
    ...
    try {
      ...
      const { data: orderData } = await api.post("/tokens/create-order", {
      ...
      const options = {
        ...
        name: "SponzaMe",
        description: `${selectedPlan.name} Plan — ${selectedPlan.tokens} Tokens`,
        order_id: orderData.order_id,
        handler: async function (response) {
          // 4. Verify payment on backend
          try {
            const { data: verifyData } = await api.post(
              "/tokens/verify-payment",
              {
      ...
      <div className="w-full px-4 sm:px-6 lg:px-8 py-8 -mt-4 relative z-20">
        <div className="max-w-5xl mx-auto">
          <div className="grid lg:grid-cols-5 gap-8">
            {/* Left Column - Plan Selection & Pay Button */}
                  ...
                  <div className="flex items-start gap-4">
                    <div className="w-12 h-12 rounded-xl bg-white shadow-sm flex items-center justify-center flex-shrink-0">
                      <svg viewBox="0 0 24 24" className="w-7 h-7" fill="none">
                        <path
                          d="M7.076 2L4 12.648l8.29-.002L7.076 2z"
                          fill="#072654"
                        />
                        <path
                          d="M14.59 7.848L12.29 12.646H20l-2.227-8.294-3.183 3.496z"
                          fill="#072654"
                        />
                      </svg>
                    </div>
                    <div>
                      ...
                      <p className="text-sm text-gray-600">
                        ...
                        Wallets, and more. All payment methods available in the
                        checkout popup.
                      </p>
                    </div>
                  </div>
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="reveal delay-2 bg-white rounded-2xl shadow-xl shadow-gray-200/50 border border-gray-100 p-6 lg:sticky lg:top-24">
                ...
                {/* Selected Plan */}
                <div
                  className={`bg-gradient-to-br ${selectedPlan.color} rounded-xl p-5 text-white mb-6`}
                >
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-white/80 text-sm">
                          {selectedPlan.perToken} per token
                        </p>
                      </div>
                    </div>
                  </div>
