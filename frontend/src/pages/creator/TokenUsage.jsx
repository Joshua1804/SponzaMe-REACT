...
import { Coins, PartyPopper, TrendingDown, ShoppingCart, ScrollText, Calendar, CheckCircle, Gift, ClipboardList } from "lucide-react";
        ...
        api.get("/creator/tokens")
            .then(res => {
                ...
                const txs = (res.data.transactions || []).map((t, i) => ({
                    ...
                    icon: t.type === "purchased" ? <ShoppingCart size={18} /> : t.type === "earned" ? <PartyPopper size={18} /> : <ClipboardList size={18} />,
                }));
                setTransactions(txs);
                setStats({
            ...
            <div className="w-full px-4 sm:px-6 lg:px-8 -mt-8 relative z-20">
                <div className="max-w-5xl mx-auto">
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        {[
                            { label: "Total Earned", value: stats.totalEarned, icon: <PartyPopper size={20} />, color: "from-emerald-400 to-green-500" },
                            { label: "Total Spent", value: stats.totalSpent, icon: <TrendingDown size={20} />, color: "from-red-400 to-pink-500" },
                            { label: "Total Purchased", value: stats.totalPurchased, icon: <ShoppingCart size={20} />, color: "from-blue-400 to-indigo-500" },
                        ].map((stat, i) => (
                            <div
                                key={i}
                            ...
                            >
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-2xl font-bold text-gray-900">{stat.value} <Coins size={16} className="inline" /></p>
                                    </div>
                                </div>
                            </div>
                        ))}
                                        ...
                                        >
                                            <div className="flex items-center justify-between">
                                                ...
                                                <div className={`text-lg font-bold ${tx.amount > 0 ? "text-emerald-600" : "text-red-500"
                                                    ...
                                                    {tx.amount > 0 ? "+" : ""}{tx.amount} <Coins size={14} className="inline" />
                                                </div>
                                            </div>
                                        </div>
                                    ))}
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="reveal delay-4 bg-white rounded-2xl shadow-xl shadow-gray-200/50 border border-gray-100 p-6">
                                ...
                                <div className="space-y-3">
                                    {[
                                        { text: "Complete campaigns successfully", icon: <CheckCircle size={16} />, tokens: "+10-20" },
                                        { text: "Refer other creators", icon: <Gift size={16} />, tokens: "+10" },
                                        { text: "Daily login bonus", icon: <Calendar size={16} />, tokens: "+1" },
                                        { text: "Profile verification", icon: <CheckCircle size={16} />, tokens: "+5" },
                                    ].map((item, i) => (
